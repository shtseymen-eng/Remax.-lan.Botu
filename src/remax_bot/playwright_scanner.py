...
def parse_remax_detail(*, text: str, url: str, source_url: str) -> Listing:
    ...
    if phone:
        for i,x in enumerate(lines):
            if re.search(r"(?:\+90\s*)?(?:0\s*)?5\d{2}",x):
                for cand in reversed(lines[max(0,i-5):i]):
                    if 4<=len(cand)<=80 and not re.search(r"\d",cand) and not re.search(r"iletişim|sertifika|re/max|emlak endeksi",cand,re.I):
                        advisor=re.sub(r"\s+(Çarşı(?: 2)?)$",r" \1",cand).strip(); break
                break
    location=""
    for x in lines:
...
def parse_listing_detail(*, text: str, url: str, source_url: str, title: str = "", price: str = "", advisor: str = "", phone: str = "") -> Listing:
    ...
    if not advisor:
        adv_match = re.search(r"(?:Danışman|İlan Sahibi|İlan sahibi)\s*[:\n]\s*([^\n]+)", text, re.I)
        advisor = adv_match.group(1).strip() if adv_match else ""
    if not phone:
        phone = _extract_phone(text)
...
class PlaywrightScanner(QObject):
    ...
    def _ensure_not_blocked(self, page) -> bool:
        ...
        if is_human_verification(text):
            return self._wait_for_user(
                "RE/MAX güvenlik doğrulaması istiyor. Açık tarayıcıda işlemi tamamlayın ve DEVAM ET'e basın."
            )
        return True
    ...
    def _open_list(self, page, source_url: str) -> None:
        ...
        self.status.emit("Chrome açıldı. RE/MAX ilan listesi yükleniyor...")
    ...
    def _read_detail(self, page, source_url: str) -> Listing | None:
        ...
        return parse_listing_detail(text=body, url=page.url, source_url=source_url, title=title, price=price, advisor=advisor, phone=phone)
        ...
        try:
            ...
            with sync_playwright() as playwright:
                ...
                if self._stop.is_set():
                    ...
                    return
                if not rows:
                    raise RuntimeError("Hiç ilan analiz edilemedi. Chrome penceresini kontrol edin.")
                if total and len(rows) != total:
                    raise RuntimeError(f"Mağazada {total} ilan görünüyor; {len(rows)} ilan analiz edildi. Eksik güncelleme yapılmadı.")
